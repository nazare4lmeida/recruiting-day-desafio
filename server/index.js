...
// ALTERADO: Agora envia regras de limite e quantas tentativas o usuário atual possui
app.get("/api/challenge", (req, res) => {
  // Buscamos o ID do participante via query string para saber se ele já enviou algo
  // Ex: /api/challenge?participantId=p_123
  const pId = req.query.participantId;
  const p = state.participants.get(pId);

  // Se houver registros na lista de codeResults, consideramos que ele já fez a tentativa
  const attemptsDone = p && p.codeResults && p.codeResults.length > 0 ? 1 : 0;
  ...
  res.json({
    ...
    maxAttempts: 1, // Configuração de tentativa única injetada dinamicamente
    attemptsDone: attemptsDone, // Informa se o usuário que requisitou já gastou a chance dele
  });
});
...
app.post("/api/register", (req, res) => {
  ...
  const participant = {
    ...
    iaAnswers: [],
    codeScore: 0,
    codeResults: [], // Histórico de testes salvos fica aqui
    startedAt: Date.now(),
    finishedAt: null,
  };
...
// ALTERADO: Rota de submissão de código agora impede reenvios
app.post("/api/code/submit", (req, res) => {
  const { participantId, code } = req.body;
  const p = state.participants.get(participantId);
  if (!p) return res.status(404).json({ error: "Participante não encontrado" });

  // TRAVA DO SERVIDOR: Se o array codeResults já possuir itens, barra o processo imediatamente
  if (p.codeResults && p.codeResults.length > 0) {
    return res.status(403).json({
      error: "Tentativas esgotadas para este desafio.",
      isLocked: true,
    });
  }
  ...
  try {
    ...
    if (typeof userFn !== "function")
      throw new Error("Função 'parOuImpar' não encontrada.");
  } catch (e) {
    // Mesmo se der erro de compilação, salvamos uma falha nos registros para contar como a única tentativa consumida
    p.codeResults = CHALLENGE.tests.map((t) => ({
      name: t.name,
      passed: false,
    }));
    ...
    return res.json({
      ...
      results: CHALLENGE.tests.map((t) => ({
        name: t.name,
        passed: false,
        error: "Código não executou",
      })),
      earned: 0,
      totalCode: p.codeScore,
      isLocked: true, // Avisa o front para bloquear tudo
    });
  }
  ...
  // Salva os resultados no objeto do participante (isso vai travar novas chamadas e servirá para o Excel)
  p.codeResults = results.map((r) => ({ name: r.name, passed: r.passed }));
  p.finishedAt = Date.now();

  broadcastRanking();

  // Retorna os dados com "isLocked: true" para sincronizar com o comportamento do front-end que alteramos
  res.json({
    results,
    earned,
    totalCode: p.codeScore,
...
app.post("/api/admin/advance", (req, res) => {
  ...
  const cutoffs = {
    1: state.cutoff.stage1,
    2: state.cutoff.stage2,
    3: state.cutoff.stage3,
  };
  const keep = cutoffs[stage];
  if (!keep) return res.status(400).json({ error: "Etapa inválida" });

  const ranked = ranking().filter(
    (p) => !state.participants.get(p.id).eliminated,
  );
...
app.get("/api/admin/export.xlsx", requireAdmin, async (req, res) => {
  ...
  const sorted = [...state.participants.values()]
    .map((p) => {
      ...
      const tempo = p.finishedAt
        ? Math.round((p.finishedAt - p.startedAt) / 1000)
        : "—";
  ...
  sorted.forEach(({ p, total, tempo }, i) => {
    ...
    if (i === 0)
      row.eachCell((c) => {
        c.fill = { type: "pattern", pattern: "solid", fgColor: gold };
      });
    if (i === 1)
      row.eachCell((c) => {
        c.fill = { type: "pattern", pattern: "solid", fgColor: silver };
      });
    if (i === 2)
      row.eachCell((c) => {
        c.fill = { type: "pattern", pattern: "solid", fgColor: bronze };
      });
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="recruiting-day-resultados.xlsx"',
  );
...
server.listen(PORT, () => {
  ...
  console.log(
    `   • Admin user: ${ADMIN_USER} (defina ADMIN_PASSWORD no .env)\n`,
  );
});
